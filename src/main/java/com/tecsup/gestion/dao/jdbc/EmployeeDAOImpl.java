...
//		Object[] params = new Object[] { password, lastname, firstname, salary, dptId, login };
	...
	public List<Employee> findEmployeesByName(String name) throws DAOException, EmptyResultException {
		...
		String query = "SELECT employee_id, login, password, first_name, last_name, salary, department_id "
				+ " FROM employees E,  WHERE upper(first_name) like upper(?) ";
	...
	/*@Override
	public List<Employee> findAllEmployeeswithRole() throws DAOException, EmptyResultException {
		String query = "SELECT E.employee_id, E.login, E.password, E.first_name, E.last_name, E.salary, E.department_id , R.role, R.description  FROM employees E, roles R";
				
				//, role, description
		
				try {
		
					List<Employee> employees = jdbcTemplate.query(query, new EmployeewithRolesMapper());
					//
					return employees;
	...
	public class EmployeewithRolesMapper implements RowMapper<Employee>{
		...
		public Employee mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			Role rols = new Role();
			rols.setRoleId(rs.getString("roles.role"));
			rols.setDescription(rs.getString("roles.description"));
			emp.setRole(rols);
